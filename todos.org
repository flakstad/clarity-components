...
* TODO datastar: testing driving with backend signals
Setup backend with in-memory state.
** TODO handle new html reinit todolist script
** TODO comm with server listening for events
* TODO datastar: using debounce on saving changes to avoid storing temp states
making it so that users can cycle through status labels without saving all temp states.

Probably best handled by datastar using debounce - so we emit all the events
right away, but we only save to backend after a delay and only use the latest
value. In this model I think we need to save the entire JSON, not just the
event. Hm tricky, I don't see fully how it can work right now..

Could have debounce on every specific event perhaps, so we save all changes to
various fields, but delay a bit to catch temp changes.
* TODO double check that I'm happy with outline item data format
..will at some point forever by locked to this structure
* TODO agenda view rendering should not be a web component?
...
We might need the agenda web component for navigation and shortcuts for queries. Or possibly datastar signals in the html is enough to detect this?
...
* TODO allow custom key bindings for all keyboard interactions?
Pass in override key bindings.
First for movement only, i.e. moving focus, moving items
* TODO space to set status as well?
* TODO drag and drop?
* TODO bug: attempt again to fix issue with hover items showing on parents when hovering child
* TODO bug: counter for nested parent without status label
